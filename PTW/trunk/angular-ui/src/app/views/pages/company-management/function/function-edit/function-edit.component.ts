...
export class FunctionEditComponent implements OnInit, OnDestroy {
	...
	initUser() {
		this.createForm();
		if (!this.user.id) {
			this.subheaderService.setTitle('Create function');
			this.subheaderService.setBreadcrumbs([
				{ title: 'Function Management', page: `function-management` },
				{ title: 'Function',  page: `function-management/function` },
				{ title: 'Create function', page: `function-management/function/add` }
			]);
			return;
		}
		this.subheaderService.setTitle('Edit function');
		this.subheaderService.setBreadcrumbs([
			{ title: 'Function Management', page: `function-management` },
			{ title: 'Function',  page: `function-management/function` },
			{ title: 'Edit function', page: `function-management/function/edit`, queryParams: { id: this.user.id } }
		]);
	}
	 ...
	 */
	goBackWithId() {
		const url = `${this.layoutConfigService.getCurrentMainRoute()}/function-management/function`;
	...
	refreshUser(isNew: boolean = false, id = 0) {
		...
		url = `${this.layoutConfigService.getCurrentMainRoute()}/function-management/function/edit/${id}`;
		this.router.navigateByUrl(url, { relativeTo: this.activatedRoute });
	}
	...
	addUser(_user: User, withBack: boolean = false) {
		...
		const addSubscription = this.store.pipe(select(selectLastCreatedUserId)).subscribe(newId => {
			const message = `New function successfully has been added.`;
	...
	updateUser(_user: User, withBack: boolean = false) {
		...
		const message = `Function successfully has been saved.`;
	...
	getComponentTitle() {
		...
		result = `Edit function - ${this.user.fullname}`;
		return result;
	}
