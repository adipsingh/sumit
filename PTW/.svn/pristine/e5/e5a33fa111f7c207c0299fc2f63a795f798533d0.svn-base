...
export const selectCustomersState = createFeatureSelector<CustomersState>('company');
...
export const selectCustomersInStore = createSelector(
    selectCustomersState,
    customersState => {
        const items: CertificateQAModel[] = [];
        each(customersState.entities, element => {
            items.push(element);
        });
        const httpExtension = new HttpExtenstionsModel();
        const result: CertificateQAModel[] = httpExtension.sortArray(items, customersState.lastQuery.sortField, customersState.lastQuery.sortOrder);
