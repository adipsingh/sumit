...
export class CompanyEditComponent implements OnInit, OnDestroy {
	...
	initUser() {
		this.createForm();
		if (!this.user.id) {
			this.subheaderService.setTitle('Create company');
			this.subheaderService.setBreadcrumbs([
				{ title: 'Company Management', page: `company-management` },
				{ title: 'Company',  page: `company-management/company` },
				{ title: 'Create company', page: `company-management/company/add` }
			]);
			return;
		}
		...
		this.subheaderService.setBreadcrumbs([
			{ title: 'Company Management', page: `company-management` },
			{ title: 'Company',  page: `company-management/company` },
			{ title: 'Edit Company', page: `company-management/company/edit`, queryParams: { id: this.user.id } }
		]);
	}
	 ...
	 */
	goBackWithId() {
		const url = `${this.layoutConfigService.getCurrentMainRoute()}/company-management/company`;
	...
	refreshUser(isNew: boolean = false, id = 0) {
		...
		url = `${this.layoutConfigService.getCurrentMainRoute()}/company-management/company/edit/${id}`;
		this.router.navigateByUrl(url, { relativeTo: this.activatedRoute });
	}
	...
	addUser(_user: User, withBack: boolean = false) {
		...
		const addSubscription = this.store.pipe(select(selectLastCreatedUserId)).subscribe(newId => {
			const message = `New company successfully has been added.`;
	 ...
	 */
	getComponentTitle() {
		let result = 'Create company';
		if (!this.user || !this.user.id) {
			return result;
		}

		result = `Edit company - ${this.user.fullname}`;
		return result;
	}
